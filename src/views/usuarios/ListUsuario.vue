        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há Usuario cadastrados. Adicione uma Usuario.
            </p>
          </v-col>
        </v-row>
          ...
          >
            ...
            >
              <span
                ...
                onclick=""
              >
                clique aqui para cadastrar seu primeiro Usuario
              </span>
            </Button>
          </v-col>
        </v-row>
...
<v-col cols="4">
        ...
        <v-text-field id="email" v-model="filtro.email" bg-color="white" placeholder="Filtrar pelo Email" rounded variant="outlined" required class="w-100 mt-5 pr-10" type="text" color="primary" />
    </v-col>
<v-col cols="4">
        <label for="password" class="small-label">Password</label>
        <v-text-field id="password" v-model="filtro.password" bg-color="white" placeholder="Filtrar pelo Password" rounded variant="outlined" required class="w-100 mt-5 pr-10" type="text" color="primary" />
    </v-col>
<v-col cols="4">
        <label for="cpf" class="small-label">Cpf</label>
        <v-text-field id="cpf" v-model="filtro.cpf" bg-color="white" placeholder="Filtrar pelo Cpf" rounded variant="outlined" required class="w-100 mt-5 pr-10" type="text" color="primary" />
    </v-col>
              </v-row>
              <v-row justify="end">
                <v-col
          ...
          <v-table class="bg-transparent w-100 mt-16">
            <thead>
              <tr>
                ...
                <th class="bolder-primary-color font-weight-bold table-head-border">Email</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Password</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Cpf</th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="(usuario, index) in response?.data"
                :key="index"
              >
                <td class="table-foot-border table-content-text" :title="usuario?.id">
                           {{ usuario?.id }}
                         </td>
                <td class="table-foot-border table-content-text" :title="usuario?.nome">
                           {{ usuario?.nome }}
                         </td>
                <td class="table-foot-border table-content-text" :title="usuario?.email">
                           {{ usuario?.email }}
                         </td>
                <td class="table-foot-border table-content-text" :title="usuario?.password">
                           {{ usuario?.password }}
                         </td>
                <td class="table-foot-border table-content-text" :title="usuario?.cpf">
                           {{ usuario?.cpf }}
                         </td>
              </tr>
            </tbody>
      ...
      >
        <v-alert type="info">
          Nenhum Usuario encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
export default {
  ...
  data(): {
    filtro: IFiltrarUsuario;
    initialAccess: boolean;
    
    response: PagedResponse<IUsuario>;
    openedPanel: number;
  ...
  computed: {
    filtroVazio() {
      return !this.filtro.nome && !this.filtro.email && !this.filtro.password && !this.filtro.cpf;
    },
  },
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Usuario. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await UsuarioService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Usuario. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
