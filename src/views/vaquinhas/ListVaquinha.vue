<template>
  <v-container>
        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há vaquinhas cadastradas. Adicione uma Vaquinha.
            </p>
          </v-col>
        </v-row>
        ...
        >
          ...
          >
            <Button
              ...
              @click="() => $router.push('/vaquinhas/cadastrar')"
            >
              <span
                onclick=""
                class="span-class"
              >
                clique aqui para cadastrar sua primeira vaquinha
              </span>
            </Button>
          </v-col>
        </v-row>
    ...
    >
      ...
      >
        ...
        <div class="scroll-table-container">
          <v-table class="bg-transparent w-100 mt-16">
            ...
            <tbody>
              <tr
                v-for="(vaquinha, index) in response?.data"
                :key="index"
              >
                <td
                  class="table-foot-border table-content-text"
                  :title="vaquinha?.id"
                >
                  {{ vaquinha?.id }}
                </td>
                <td
                  class="table-foot-border table-content-text"
                  :title="vaquinha?.pessoa_id"
                >
                  {{ vaquinha?.pessoa?.nome }}
                </td>
                <td class="table-foot-border table-content-text">
                  {{ `${vaquinha?.endereco_itens_ajuda?.logradouro} - ${vaquinha?.endereco_itens_ajuda?.bairro} - ${vaquinha?.endereco_itens_ajuda?.cidade?.nome}` }}
                </td>
                <td
                  class="table-foot-border table-content-text"
                  :title="vaquinha?.descricao_objetivo"
                >
                  {{ truncateText(vaquinha?.descricao_objetivo, 50) }}
                </td>
                <td
                  class="table-foot-border table-content-text"
                  :title="vaquinha?.status"
                >
                  {{ vaquinha?.status }}
                </td>
              </tr>
              ...
              <td class="table-foot-border text-center table-content-text">
                <i
                  ...
                  @click="
                    () =>
                      $router.push(
                        `/vaquinhas/visualizar/${vaquinha.id}`
                      )
                  "
                />
                <i
                  v-if="shouldShowEdit(vaquinha)"
                  class="fas fa-edit ml-2"
                  style="color: rgb(var(--v-theme-primary)); cursor: pointer"
                  @click="
                    () =>
                      $router.push(
                        `/vaquinhas/editar/${vaquinha.id}`
                      )
                  "
                />
              </td>
      ...
      >
        <v-alert type="info">
          Nenhum Vaquinha encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
import { truncateText } from '@/mixins/masks';
...
export default {
  ...
  data(): {
    filtro: IFiltrarVaquinha;
    initialAccess: boolean;
    pessoas: IPessoa[];
    enderecos: IEndereco[];
    user: IUsuario;
    response: PagedResponse<IVaquinha>;
    openedPanel: number;
  ...
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Vaquinha. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await VaquinhaService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Vaquinha. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
