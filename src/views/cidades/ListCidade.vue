        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há Cidade cadastrados. Adicione uma Cidade.
            </p>
          </v-col>
        </v-row>
        ...
        >
          ...
          >
            <Button
              ...
              @click="() => $router.push('/cidades/cadastrar')"
            >
              <span
                class="text-wrap"
                onclick=""
              >
                clique aqui para cadastrar seu primeiro Cidade
              </span>
            </Button>
          </v-col>
        </v-row>
...
<v-col cols="4">
                <label for="estado_id" class="small-label">Estado</label>
                <v-select id="estado_id" v-model="filtro.estado_id" bg-color="white" rounded variant="outlined" placeholder="Filtrar por Estado" required class="w-100 mt-5 pr-10" color="primary" item-value="id" item-title="nome" :items="estados" />
              </v-col>
              </v-row>
              <v-row justify="end">
                <v-col
          ...
          <v-table class="bg-transparent w-100 mt-16">
            <thead>
              <tr>
                ...
                <th class="bolder-primary-color font-weight-bold table-head-border">Estado</th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="(cidade, index) in response?.data"
                :key="index"
              >
                <td class="table-foot-border table-content-text" :title="cidade?.id">
                           {{ cidade?.id }}
                         </td>
                <td class="table-foot-border table-content-text" :title="cidade?.nome">
                           {{ cidade?.nome }}
                         </td>
                <td class="table-foot-border table-content-text" :title="cidade?.estado_id">
                           {{ cidade?.estado?.nome }}
                         </td>
              </tr>
            </tbody>
      ...
      >
        <v-alert type="info">
          Nenhum Cidade encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
export default {
  ...
  data(): {
    filtro: IFiltrarCidade;
    initialAccess: boolean;
    estados: IEstado[];
    response: PagedResponse<ICidade>;
    openedPanel: number;
  ...
  computed: {
    filtroVazio() {
      return !this.filtro.nome && !this.filtro.estado_id;
    },
  },
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Cidade. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await CidadeService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Cidade. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
