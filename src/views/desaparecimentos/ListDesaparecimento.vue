<template>
  <v-container>
        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há Desaparecimento cadastrados. Adicione uma Desaparecimento.
            </p>
          </v-col>
        </v-row>
        ...
        >
          ...
          >
            <Button
              ...
              @click="() => $router.push('/desaparecimentos/cadastrar')"
            >
              <span
                onclick=""
                class="span-class"
              >
                clique aqui para cadastrar seu primeiro Desaparecimento
              </span>
            </Button>
          </v-col>
        </v-row>
    ...
    >
            ...
            >
              <v-row>
                ...
                <v-col cols="12"
                       md="4">
                  <label for="status"
                         class="small-label">Status</label>
                  <v-select id="status"
                            v-model="filtro.status"
                            bg-color="white"
                            rounded variant="outlined"
                            placeholder="Filtrar por status"
                            required class="w-100 mt-5 pr-10"
                            color="primary"
                            item-value="id"
                            item-title="nome"
                            :items="status"
          ...
          <v-table class="bg-transparent w-100 mt-16">
            <thead>
              <tr>
                ...
                <th class="bolder-primary-color font-weight-bold table-head-border">Pessoa</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Endereço</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Status</th>
                <th class="bolder-primary-color font-weight-bold table-head-border">Visto por último em</th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="(desaparecimento, index) in response?.data"
                :key="index"
              >
                ...
                <td class="table-foot-border table-content-text">
                  {{ desaparecimento?.pessoa?.nome }}
                </td>
                <td class="table-foot-border table-content-text">
                  {{ `${desaparecimento?.endereco?.logradouro} - ${desaparecimento?.endereco?.bairro} - ${desaparecimento?.endereco?.cidade?.nome}`  }}
                </td>
                <td class="table-foot-border table-content-text">
                  {{ desaparecimento?.status }}
                </td>
                <td class="table-foot-border table-content-text">
                  {{ formataDateTimeParaDataBrasileira(desaparecimento?.ultimo_visto_em) }}
                </td>
              </tr>
            </tbody>
              ...
              <td class="table-foot-border text-center table-content-text">
                <i
                  ...
                  @click="
                    () =>
                      $router.push(
                        `/desaparecimentos/visualizar/${desaparecimento.id}`
                      )
                  "
                />
                <i
                  v-if="shouldShowEdit(desaparecimento)"
                  class="fas fa-edit ml-2"
                  style="color: rgb(var(--v-theme-primary)); cursor: pointer"
                  @click="
                    () =>
                      $router.push(
                        `/desaparecimentos/editar/${desaparecimento.id}`
                      )
                  "
                />
              </td>
      ...
      >
        <v-alert type="info">
          Nenhum Desaparecimento encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
export default {
  ...
  data(): {
    filtro: IFiltrarDesaparecimento;
    initialAccess: boolean;
    pessoas: IPessoa[];
    enderecos: IEndereco[];
    status: TypeStatusDesaperecimento[];
    usuarios: IUsuario[];
    response: PagedResponse<IDesaparecimento>;
    openedPanel: number;
  ...
  computed: {
    filtroVazio() {
      return !this.filtro.pessoa && !this.filtro.cidade && !this.filtro.bairro && !this.filtro.status;
    },
    shouldShowEdit() {
      return (model) => {
  ...
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Desaparecimento. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await DesaparecimentoService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Desaparecimento. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
