...
export default {
  ...
  async mounted() {
    try {
      ...
      this.errorMessage =
        'Erro ao buscar dados de Estado. Tente novamente e em caso de persistência de erros entre em contato com o suporte.';
    } finally {
  ...
  methods: {
    ...
    async saveEstado() {
      ...
      try {
        ...
        let response;
        if (this.id) {
          response = await EstadoService.update({
            nome: this.formData.nome
          }, this.id);
        } else {
          response = await EstadoService.save(this.formData);
          this.id = response.id;
        }

        this.successMessage = "Estado salva com sucesso!";
        this.snackbar = true;
        ...
        if (error.response && error.response.status === 422) {
          ...
          this.errorSnackbar = true;
          this.errorMessage = "Houve um erro ao cadastrar Estado, valide os dados e tente novamente";
    ...
    async checkIfEditing() {
      try {
        ...
        if (this.$route?.params?.id) {
          ...
          const estado = await EstadoService.getById(this.id);
          Object.assign(this.formData, estado);
        }
