        ...
        <v-row>
          ...
          >
            <p class="empty-data-caption my-5">
              Ainda não há Pessoa cadastrados. Adicione uma Pessoa.
            </p>
          </v-col>
        </v-row>
        ...
        >
          ...
          >
            <Button
              ...
              @click="() => $router.push('/pessoas/cadastrar')"
            >
              <span
                class="text-wrap"
                onclick=""
              >
                clique aqui para cadastrar seu primeiro Pessoa
              </span>
            </Button>
          </v-col>
        </v-row>
...
<v-col cols="4">
      ...
      >
        ...
        <div class="scroll-table-container">
          <v-table class="bg-transparent w-100 mt-16">
            ...
            <tbody>
              <tr
                v-for="(pessoa, index) in response?.data"
                :key="index"
              >
                <td class="table-foot-border table-content-text" :title="pessoa?.id">
                           {{ pessoa?.id }}
                         </td>
                <td class="table-foot-border table-content-text" :title="pessoa?.nome">
                           {{ pessoa?.nome }}
                         </td>
                <td class="table-foot-border table-content-text" :title="pessoa?.cpf">
                           {{ pessoa?.cpf }}
                         </td>
              </tr>
            </tbody>
      ...
      >
        <v-alert type="info">
          Nenhum Pessoa encontrada com a sua busca. Ajuste os filtros e tente
          novamente
        </v-alert>
      </div>
    </v-container>
...
export default {
  ...
  data(): {
    filtro: IFiltrarPessoa;
    initialAccess: boolean;
    
    response: PagedResponse<IPessoa>;
    openedPanel: number;
  ...
  computed: {
    filtroVazio() {
      return !this.filtro.nome && !this.filtro.cpf;
    },
  },
  async mounted() {
    try {
      ...
      this.errorMessage = "Houve um erro ao buscar dados de Pessoa. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
  ...
  methods: {
    ...
    async filtrar() {
      try {
        ...
        this.isLoading = true;
        this.response = await PessoaService.filter(this.filtro);
      } catch (error) {
        this.errorMessage = "Houve um erro ao buscar dados de Pessoa. Tente novamente a operação e em caso de persistência do erro entre em contato com o suporte";
        this.errorSnackbar = true;
      } finally {
