...
export default {
  ...
  async mounted() {
    try {
      ...
      this.errorMessage =
        'Erro ao buscar dados de Pessoa. Tente novamente e em caso de persistência de erros entre em contato com o suporte.';
    } finally {
  ...
  methods: {
    ...
    async savePessoa() {
      ...
      try {
        ...
        let response;
        if (this.id) {
          response = await PessoaService.update({
            nome: this.formData.nome,
            cpf: this.formData.cpf
          }, this.id);
        } else {
          response = await PessoaService.save(this.formData);
          this.id = response.id;
        }

        this.successMessage = "Pessoa salva com sucesso!";
        this.snackbar = true;
        ...
        if (error.response && error.response.status === 422) {
          ...
          this.errorSnackbar = true;
          this.errorMessage = "Houve um erro ao cadastrar Pessoa, valide os dados e tente novamente";
    ...
    async checkIfEditing() {
      try {
        ...
        if (this.$route?.params?.id) {
          ...
          const pessoa = await PessoaService.getById(this.id);
          Object.assign(this.formData, pessoa);
        }
